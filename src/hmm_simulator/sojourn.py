"""Sojourn time distributions for the HMM simulator.

Implements piecewise-exponential survival functions to sample the time
a subject spends in a given state before transitioning.
"""
...
def kappa_0(init_age, current_state, t) -> float:
    grp = age_group_idx(init_age + t)
    s = sum(legal_transitions(current_state, lambda_sr[grp], norm=False))
    return -1.0 * t * s
...
def kappa_1(age, current_state, t) -> float:
    ...
    grp = age_group_idx(age + t)
    tau_l = age_partitions[grp][0]

    s_k = sum(legal_transitions(current_state, lambda_sr[k], norm=False))
    s_l = sum(legal_transitions(current_state, lambda_sr[grp], norm=False))
...
def kappa_m(age, current_state, m) -> float:
    ...
    s_kmm = sum(
        legal_transitions(current_state, lambda_sr[kmm, :], norm=False)
    )
...
def eval_sojourn_time(n: int, age: int, current_state: int, t: int) -> float:
    """Evaluate the CDF for the sojourn time at time t.
...
def sojourn_time_cdf(start_age, stop_age, current_state) -> np.ndarray:
    ...
    for t in range(1, time_lapse):
        grp = age_group_idx(start_age + t)
        cdf[t] = eval_sojourn_time(grp - k, start_age, current_state, t)
...
def sojourn_time(start_age: int, age_max: int, current_state: int) -> float:
    ...
    # Step 3.
    indices = np.where(u > sojourn_cdf)[0]
    t_lower = indices[-1] if len(indices) > 0 else 0

    # Shift time point by age to satisfy grp: P(T < tau_grp - a) < u.
    grp = age_group_idx(start_age + t_lower)

    # NB: Adjust to Python count logic and start sum from 1.
    sum_k = sum(
        kappa(start_age, current_state, start_age + t_lower, i)
        for i in range(1, grp - k)
    )
    sum_p = sum(legal_transitions(current_state, lambda_sr[grp, :]))
